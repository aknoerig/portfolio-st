...
// gibt die Dateiendung einer Datei zur�ck
...
function createThumbnail($pic, $thumbdir, $thumbname, $newwidth, $quality)
{
  ...
  // Gr��e berechnen
  // kein resize, wenn die aktuelle Bildh�he weniger als die neue Bildh�he betr�gt
  if($size[1] < $newwidth){
    $width = $size[0];
    $height = $size[1];
  }
  // Bild nach H�he anpassen
  else{
    $width = $newwidth;
  ...
  if($size[2]==1) {
    ...
    // Originalbild in das Thumbnail mit angepasster Gr��e kopieren
    ImageCopyResized($thumbnail,$original,0,0,0,0,$width,$height,$size[0],$size[1]);
    // Bild im tempor�ren Thumbnail Ordner speichern
...
{
  // pr�fen ob ein '/' am Ende des Ordnernamens steht
  if(substr($source,-1)!="/")
  $source .= "/";
  if(substr($target,-1)!="/")
  $target .= "/";
